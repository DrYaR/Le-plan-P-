...
int pcMenu(Player *player) {
    ...
    if (choice == '1') {
        ...
        if (player->hour <= 19) {player->hour += 4;}
...
int lectureMenu(Player *player) {
    ...
    printf("\nChoose task for now:\n");
    printf("1. Слушать лекцию (-30 минут, -5 Энергия)\n");
...
int speakMenu(Player *player) {
    ...
    printf("\nChoose task for now:\n");
    printf("1. Провести лекцию (-30 минут, -10 Энергия)\n");
...
if (player->mental >= 30){
    ...
    if (newX >= 0 && newX < width && newY >= 0 && newY < height && map[newY][newX] != '1' && map[newY][newX] != '2' && map[newY][newX] != '3' && map[newY][newX] != '4' && map[newY][newX] != '5' && map[newY][newX] != '7' && map[newY][newX] != '6' && map[newY][newX] != '#' && map[newY][newX] != '8' && map[newY][newX] != 'b' && map[newY][newX] != 'y' ) {
        player->x = newX;
        player->y = newY;
    }
}
...
void movePlusSymbols(char **map, int width, int height) {
     srand(time(NULL));
     for (int y = 0; y < height; y++) {
         for (int x = 0; x < width; x++) {
             if (map[y][x] == '+') {
...
int main() {
    ...
    movePlusSymbols(map, width, height);
